...
function scrappad.numberBox(x, y, key, titleText)
    ...
    local option = widgetsAreUs.textBox(x + 55, y, 25, 25, {1, 1, 1}, 0.9, "num", 1, 0, 0)
    local value
...
function scrappad.checkboxFullLine(x, y, key, titleText)
    ...
    local value
    return widgetsAreUs.attachCoreFunctions({title, key = key, value = value, check = check, setValue = function(newValue) check.check.setText(newValue) end })
end
